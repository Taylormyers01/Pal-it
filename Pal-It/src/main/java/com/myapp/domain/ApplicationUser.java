...
public class ApplicationUser implements Serializable {
    ...
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "paintFormulas", "applicationUser" }, allowSetters = true)
    private Set<Formula> formulaNames = new HashSet<>();

    @OneToMany(mappedBy = "applicationUser")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "miniatureFormulas", "applicationUser" }, allowSetters = true)
    ...
    public void setFormulaNames(Set<Formula> formulas) {
        if (this.formulaNames != null) {
            this.formulaNames.forEach(i -> i.setApplicationUser(null));
        }
        if (formulas != null) {
            formulas.forEach(i -> i.setApplicationUser(this));
        }
    ...
    public void setMiniatureNames(Set<Miniature> miniatures) {
        if (this.miniatureNames != null) {
            this.miniatureNames.forEach(i -> i.setApplicationUser(null));
        }
        if (miniatures != null) {
            miniatures.forEach(i -> i.setApplicationUser(this));
        }
